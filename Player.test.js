...
test('Crea un jugador sin ranking, se debe poner 500 por defecto', () => {
  const player1 = new Player('Manolo')
  expect(player1.name).toBe('Manolo')
...
test('Crea un jugador de género feminino', () => {
  const player1 = new Player('Selena', 12, 'female')
  expect(player1.genre).toBe('female')
})

test('Crea un jugador de género feminino si no se pone nada', () => {
  const player1 = new Player('Selena', 12)
  expect(player1.genre).toBe('female')
})

test('Crea un jugador de género femenino si no es male o female', () => {
  const player1 = new Player('Selena', 12, 'queer')
  expect(player1.genre).toBe('female')
})

test('Crear un jugador con fecha nacimiento', () => {
  const player1 = new Player('Selena', 12, 'female', '4/8/97')
  expect(player1.birthday).toStrictEqual(new Date(1997, 7, 4))
})

test('Crear un jugador sin fecha nacimiento poner por defecto 1/1/2000', () => {
  const player1 = new Player('Selena', 12, 'female')
  expect(player1.birthday).toStrictEqual(new Date(2000, 0, 1))
})

test('Crear un jugador con fecha nacimiento no incorrecta', () => {
  const player1 = new Player('Selena', 12, 'female', '33/14/2000')
...
test('Se crea jugador con fecha y muestra su edad corrctamente', () => {
